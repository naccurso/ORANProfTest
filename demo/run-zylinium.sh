...
curl -i -X PUT -H "Content-type: application/json" -d '{"ul_mask_sched":[{"mask":"0xf00000","start":'`echo "import time; print(time.time() + 60)" | python`'},{"mask":"0xfe0000","start":'`echo "import time; print(time.time() + 120)" | python`'},{"mask":"0xf00000","start":'`echo "import time; print(time.time() + 180)" | python`'},{"mask":"0xfe0000","start":'`echo "import time; print(time.time() + 240)" | python`'}]}' ${NEXRAN_URL}/v1/nodebs/enB_macro_001_001_00019b ; echo ; echo ;
